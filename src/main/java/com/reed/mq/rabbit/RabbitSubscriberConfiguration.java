 ...
 * <pre>
 * Spring构造器注入。
 * 在spring XML配置文件中配置一个实例就可以启动一个监听实例，需要监听多个队列或者配置不同的handler时只需多陪只几个实例即可
 * </pre>
 * 
 */
public class RabbitSubscriberConfiguration extends
	...
	public RabbitSubscriberConfiguration(ConnectionFactory connectionFactory,
			String queueName, RabbitSubscriberMessageHandler messageHandler) {
